...
{
    if (PlayerSwitch && !UIcanvas.activeInHierarchy && Vector3.Angle(transform.up, Vector3.up) < 100)
    {
        if (currentAmmo > 0)
        {
            ...
            Shoot(); // <-- CALL THIS TO FIRE BULLET
        }
        else
        {
            if (emptyGunShot != null && audioSource != null)
            {
    ...
    void Update()
    {
        ...
        if (PlayerSwitch && !UIcanvas.activeInHierarchy &&
            Vector3.Angle(transform.up, Vector3.up) > 100 &&
            currentAmmo < maxAmmo)
        {
            Reload();
        }
